...
interface PortStore {
  ...
  updatePortPosition: (id: PortId, position: { x: number; y: number }) => void;
  updatePortsPositionBatch: (updates: { id: PortId; x: number; y: number }[]) => void;
...
export const usePortStore = create<PortStore>((set, get) => ({
  ...
  updatePortPosition: (id, position) => {
    set((state) => {
      ...
      if (!prev) return { ports: state.ports };
      return {
        ports: {
          ...state.ports,
          [id]: { ...prev, position }
  ...
  updatePortsPositionBatch: (updates) => {
    set((state) => {
      if (!updates?.length) return { ports: state.ports };
      const next = { ...state.ports };
      for (const { id, x, y } of updates) {
        const p = next[id];
        if (p) next[id] = { ...p, position: { x, y } };
      }
      return { ports: next };
    });
  },
