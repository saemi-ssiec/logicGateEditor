...
const EditorPage: React.FC = () => {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (nodeType === 'tag') {
      ...
      const isTriangleGate = 
        gateType === 'NOT' || 
        gateType === 'COMPARATOR' ||
        gateType === 'RISING' ||
        gateType === 'FALLING';
      const leftAnchorOffset = isTriangleGate ? Math.max(4, newNode.size.height * 0.10) : 0;
      
      const hasOutputBubble = 
        gateType === 'NOT' ||
        gateType === 'NAND' ||
        gateType === 'NOR';
      const bubbleOffset = hasOutputBubble ? Math.max(3, newNode.size.height * 0.08) + 2 : 0;
      ...
      gatePorts.push({
        ...
        position: { x: newNode.size.width + bubbleOffset, y: newNode.size.height / 2},
        connected: false
      });
